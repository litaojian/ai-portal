...
export function DynamicTable({ config, data }: DynamicTableProps) {
  const [sorting, setSorting] = React.useState<SortingState>([]);

  // Dynamically generate columns based on config and registry
  const columns = React.useMemo<ColumnDef<any>[]>(() => {
    return config.views.table.columns.map((colConfig) => {
      // 1. Get field definition from model
      const fieldDef = config.model.fields[colConfig.key];
      
      // 2. Determine renderer from registry based on type or specific component override
      // Priority: View Config Component > Field Type > Default Text
      ...
      return {
        ...
        header: () => {
          return colConfig.label || fieldDef?.label || colConfig.key;
        },
        cell: ({ row }) => {
          ...
          return (
            <Renderer 
              ...
              config={colConfig} // Pass column config (width, etc.) if needed
            />
          );
        },
