...
export function ProjectSheet({ open, onOpenChange, project }: ProjectSheetProps) {
  ...
  return (
    <Sheet open={open} onOpenChange={onOpenChange}>
      <SheetContent className="sm:max-w-[540px] overflow-y-auto px-10 py-6">
        <SheetHeader>
          <SheetTitle className="text-xl font-semibold">{project ? "编辑项目" : "新建项目"}</SheetTitle>
          <SheetDescription>
            {project ? "修改现有项目信息。" : "创建一个新的项目来跟踪进度。"}
          </SheetDescription>
        </SheetHeader>
        <Form {...form}>
